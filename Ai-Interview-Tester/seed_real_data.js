...
const dbPath = path.join(__dirname, 'backend', 'interview_system.db');
...
try {
    ...
    const insertAptitude = (dataset, topic) => {
        ...
        for (const row of dataset) {
            ...
            // Random difficulty to distribute among easy/medium/hard so the frontend logic can pull them
            ...
            count++;
            if (count >= 100) break; // Limit to 100 so it doesn't overload DB randomly
        }
    ...
    insertAptitude(verbalData, 'verbal'); // CSE is mapped to verbal for system compatibility
    ...
    for (const prob of leetcodeData) {
        ...
        // Remove HTML tags for clean display if possible or keep as is.
        // We will keep a cleaner snippet of statement up to 2000 chars or full.
        ...
        codingCount++;
        if (codingCount >= 50) break; // Only take top 50 to keep the system fast
    }

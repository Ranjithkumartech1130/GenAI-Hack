...
import { GENERAL_QUESTIONS_POOL } from "@/constants/hr-questions";
...
${/hr/i.test(type) ? "This is an HR round, so include some general behavioral and logical questions like " + GENERAL_QUESTIONS_POOL.slice(0, 3).join(", ") : ""}`,
    });
  ...
  if (!questionsList) {
    ...
    if (/behavioral/i.test(type) || /mixed/i.test(type) || /hr/i.test(type)) {
      // Prioritize some custom HR questions from our dataset pool
      const hrSamples = [...GENERAL_QUESTIONS_POOL].sort(() => 0.5 - Math.random());
      pool.push(...hrSamples.slice(0, 5));
      pool.push(...templatesBehavioral);
    }

    // Ensure enough questions
    while (pool.length < total) {
      if (/hr/i.test(type) || /behavioral/i.test(type)) {
        const extraHr = GENERAL_QUESTIONS_POOL[Math.floor(Math.random() * GENERAL_QUESTIONS_POOL.length)];
        if (!pool.includes(extraHr)) pool.push(extraHr);
        else pool.push(`What would you improve in your approach to ${role}`);
      } else {
